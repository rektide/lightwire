...
async fn main() -> Result<()> {
    ...
    match cli.command {
        Commands::Populate(_opts) => run_populate(cli.dry_run).await?,
...
async fn run_populate(_dry_run: bool) -> Result<()> {
    println!("Populate command");
    Ok(())
}

async fn run_sync_to_pipewire(_dry_run: bool) -> Result<()> {
    println!("Sync to PipeWire command");
    Ok(())
}

async fn run_sync_to_light(_dry_run: bool) -> Result<()> {
    println!("Sync to Light command");
    Ok(())
}
